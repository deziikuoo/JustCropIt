...
class ImageWorkerPool {
  ...
  /**
   * Determine optimal pool size based on hardware capabilities
   */
  private determinePoolSize(): number {
    ...
    // Reserve 1 core for the main thread (UI) to prevent jank
    let optimalCount = Math.max(1, cpuCores - 1);

    // Reduce concurrency if memory is low (< 4GB)
    if (memory < 4) {
      optimalCount = Math.min(optimalCount, 2);
    } else if (memory >= 8) {
      // High-end device (Pixel Pro), allow more workers but cap at max
      optimalCount = Math.min(optimalCount, 8);
    }
  ...
  /**
   * Determine if workers should be used for a given batch size
   */
  ...
  private ensureWorkers() {
    if (this.workers.length < this.maxWorkers) {
      const countToCreate = this.maxWorkers - this.workers.length;
  ...
  submitTask(
    ...
    return new Promise((resolve, reject) => {
      ...
      const task: Task = { request, resolve, reject, transferables };
  ...
  private processQueue() {
    ...
    // Find an idle worker
    // A simple way to track idle workers is to pop from the list when busy and push back when done?
    // Or just track active count if we assume round-robin.
    // For simplicity, let's just track available workers explicitly.
    // But since we created them in a list, we need to know WHICH one is free.
    // Let's change strategy: separate list for idle workers.
    
    // Actually, let's keep it simple: We need to track which workers are busy.
    // Since we don't have a sophisticated job ID map per worker in this simple implementation,
    // we can implement a queue-based distribution.
    
    // Improved implementation:
    // If activeWorkers < workers.length, it means at least one is free?
    // No, we need a list of IDLE workers.
  }
}

// Re-implementing with idle/busy tracking for correctness
