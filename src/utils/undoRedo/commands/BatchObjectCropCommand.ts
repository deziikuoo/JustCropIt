...
import { runBatchObjectCropPipeline } from '../../batchObjectCrop';
import type { BatchProgressCallback } from '../../batchEditProgress';

export class BatchObjectCropCommand extends BaseCommand {
  private indices: number[];
  private padPx: number;
  ...
  result: BatchObjectCropResult | null = null;
  ...
  async execute(): Promise<void> {
    ...
    this.result = await runBatchObjectCropPipeline(
      this.indices,
      this.photos,
      this.padPx,
  ...
  getDescription(): string {
    return `Crop to object (${this.indices.length} photos)`;
  }
